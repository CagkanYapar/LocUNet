...
simName = 'SDPR';
logName = strcat(pathResults,'/SDPR_Std',int2str(std),'_BS',int2str(noAnt),'_bmax',int2str(bmax),'.txt');
...
for tt=1:noTrials
    ...
    for mm=1:noMapTest
        ...
        for rx=1:noRx
            ...
            difTest(tt,mm,rx) = delay_DBSDPRFunCluster(shuffledM,squeeze(antList(tt,shuffledM,:)),squeeze(antX(tt,:,shuffledM)),squeeze(antY(tt,:,shuffledM)),squeeze(rxx(shuffledM,rx)),squeeze(rxy(shuffledM,rx)),squeeze(matDelayLocReshaped(:,:,shuffledM,:)),std,noAnt,bmax);
