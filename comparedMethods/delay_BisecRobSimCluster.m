function delay_BisecRobSimCluster(input_argument)
...
simName = 'BisecRob';
logName = strcat(pathResults,'/BisecRob_Std',int2str(std),'_BS',int2str(noAnt),'_bmax',int2str(bmax),'.txt');

fileID = fopen(logName,'a');
fprintf(fileID, 'BisecRob\n');
fclose(fileID);
...
for tt=1:noTrials
    for mm=1:noMapTest
        ...
        parfor (rx=1:noRx, Workers)
            %         for rx=1:noRx            
            difTest(tt,mm,rx) = delay_DBBisecRobFunCluster(squeeze(antList(tt,shuffledM,:)),squeeze(antX(tt,:,shuffledM)),squeeze(antY(tt,:,shuffledM)),squeeze(rxx(shuffledM,rx)),...
                squeeze(rxy(shuffledM,rx)),squeeze(matDelayLocReshaped(:,:,shuffledM,:)),std,noAnt,bmax);            
        end        
    end
    if tt==1
        toc
