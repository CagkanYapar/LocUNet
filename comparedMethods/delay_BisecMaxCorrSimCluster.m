function delay_BisecMaxCorrSimCluster(input_argument)
...
simName = 'BisecMaxCorr';
logName = strcat(pathResults,'/BisecMaxCorr_Std',int2str(std),'_BS',int2str(noAnt),'.txt');

fileID = fopen(logName,'a');
fprintf(fileID, 'BisecMaxCorr\n');
fclose(fileID);
...
for tt=1:noTrials
    ...
    for mm=1:noMapTest
        ...
        parfor (rx=1:noRx, Workers)
            ...
            difTest(tt,mm,rx) = delay_DBBisecMaxCorrFunCluster(shuffledM,squeeze(antList(tt,shuffledM,:)),squeeze(antX(tt,:,shuffledM)),squeeze(antY(tt,:,shuffledM)),squeeze(rxx(shuffledM,rx)),squeeze(rxy(shuffledM,rx)),squeeze(matDelayLocReshaped(:,:,shuffledM,:)),std,noAnt);
...
for tt=1:noTrials
    ...
    for mm=1:noMapTra
        ...
        parfor (rx=1:noRx, Workers)
            ...
            difTra(tt,mm,rx) = delay_DBBisecMaxCorrFunCluster(shuffledM,squeeze(antList(tt,shuffledM,:)),squeeze(antX(tt,:,shuffledM)),squeeze(antY(tt,:,shuffledM)),squeeze(rxx(shuffledM,rx)),squeeze(rxy(shuffledM,rx)),squeeze(matDelayLocReshaped(:,:,shuffledM,:)),std,noAnt);
